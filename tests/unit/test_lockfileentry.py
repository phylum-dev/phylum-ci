"""Test the `LockfileEntry` dataclass."""

from pathlib import Path

import pytest

from phylum.ci.common import LockfileEntry
...
def test_path_entries_are_paths(tmp_path: Path) -> None:
    ...
    depfile_entry = LockfileEntry(depfile_path_as_str, type="pip")
    assert isinstance(depfile_entry.path, Path)
    assert depfile_entry.path == depfile_path_as_path

    depfile_entry = LockfileEntry(depfile_path_as_path, type="pip")
    assert isinstance(depfile_entry.path, Path)
    assert depfile_entry.path == depfile_path_as_path

    with pytest.raises(TypeError):
        depfile_entry = LockfileEntry(depfile_path_as_bytes, type="pip")
...
def test_entries_compare_equal() -> None:
    ...
    specific_python_entry_1 = LockfileEntry("requirements.txt", "pip")
    specific_python_entry_2 = LockfileEntry("requirements.txt", "pip")
    assert specific_python_entry_1 == specific_python_entry_2

    specific_python_entry_3 = LockfileEntry("poetry.lock", type="poetry")
...
def test_auto_entries_compare_equal() -> None:
    ...
    entry_with_default_type = LockfileEntry(depfile)
    entry_with_specified_type = LockfileEntry(depfile, type="pip")
